...
public abstract class GameMode {
	...
	public GameMode(int size, BoardType type) {
		board = new Board(size, type);
	}

	// check coordinates are within bounds, cells are valid,
	// move is exactly 2 cells in a straight line, and cell states are correct
	public boolean isValidMove(int fromRow, int fromCol, int toRow, int toCol) {
		...
		if (rowDiff == 2 && colDiff == 0); // vertical move, __ok__
		else if (rowDiff == 0 && colDiff == 2); // horizontal move, __ok__
		else return false; // diagonal or wrong distance, not __ok__
	...
	// each subclass defines how a turn is played
